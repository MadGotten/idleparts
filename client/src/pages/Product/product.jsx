...
function Product() {
  const {isLoading, data, status} = useQuery(['product',window.location.pathname.slice(3)], getProduct) // Find a way to get product id in cleaner method
          ...
          <div className="flex flex-col sm:flex-row w-full gap-4">
            ...
            <div className="flex flex-col w-full sm:w-1/3 p-4 justify-between bg-slate-50 shadow-md drop-shadow-md rounded-lg">
              <div className='flex flex-col gap-2'>
                ...
                <div className="text-base">{data.stock ? (<>{`only ${data.stock} in stock`}</>):( <>Out of stock</> )}</div>
              </div>
              <div className="flex flex-col gap-4">
                <button onClick={() => addWishlist(data._id)} className="bg-orange-400 hover:bg-orange-500 rounded-lg text-center p-2 text-sm text-slate-200 whitespace-nowrap">Add to wishlist</button>
                <button onClick={() => addProduct(data._id)} className="bg-blue-600 hover:bg-blue-700 rounded-lg text-center p-3 text-base text-slate-200">Add to cart</button>
              </div>
            </div>
          </div>
        }
