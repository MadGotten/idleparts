  ...
  return (
    <div className="flex flex-col justify-center content-center gap-8 p-4 mt-12 mb-12">
     ...
     {form === 'login' ? <Login /> : <Register />}
      <button onClick={() => handleFormChange('login')} className={`${form === 'register' ? 'visible' : 'hidden'} text-sm text-slate-500`}>
        I already have an account.&nbsp;
        <span className="text-black font-medium">Sign me</span>
      </button>
      <button onClick={() => handleFormChange('register')} className={`${form === 'login' ? 'visible' : 'hidden'} text-sm text-slate-500`}>
