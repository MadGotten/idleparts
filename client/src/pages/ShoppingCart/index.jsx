      ...
      <div className="flex flex-col md:flex-row w-full items-center sm:items-start md:justify-between gap-8 md:gap-4">
                ...
                <div key={products._id} className="flex gap-4 p-4 bg-slate-50 shadow-md drop-shadow-md rounded-lg">
                  ...
                  <div className="flex flex-col justify-between w-52 lg:w-64 min-h-[8rem] items-end">
                    <div className="flex flex-row items-start gap-3">
                      ...
                      <button onClick={()=> deleteFromCart(products._id)} className="text-blue-600 rounded-lg"><i className="fa-solid fa-trash"></i></button>
                    </div>
                    <div className="flex flex-row gap-1 justify-end">
                      <div className="flex rounded-lg w-20 p-2 justify-center bg-blue-600 text-white text-sm">{products.price}$</div>
                      <div className="flex rounded-lg w-8 p-2 justify-center bg-blue-600 text-white text-sm">{cartProducts.filter(id=> id === products._id).length}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="flex flex-col w-full md:w-[18.75rem] justify-between h-40 bg-slate-50 shadow-md drop-shadow-md rounded-lg p-2">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-between font-normal text-lg">Total: <div className="flex flex-col items-end font-semibold gap-1">{total}$ <span className="font-normal text-sm text-gray-500">+ free delivery</span></div></div>
            <button onClick={()=>buyproducts()} className="flex justify-center w-full bg-blue-600 rounded-lg p-2 text-white">Pay now</button>
          </div>
        </div>
      </div>
    </div>
